...
async def create_token_endpoint(
    token_create: TokenCreate, admin: Dict[str, Any] = Depends(get_admin_token)
):
    """
...
async def delete_token(token: str, admin: Dict[str, Any] = Depends(get_admin_token)):
    """
...
async def get_usage_stats(admin: Dict[str, Any] = Depends(get_admin_token)):
    ...
    recent_activity = (
        await db.usages.find().sort("timestamp", -1).limit(10).to_list(length=10)
    )
