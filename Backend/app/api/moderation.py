...
async def moderate_image(
    ...
    try:
        moderation_results = await image_analysis_service.analyze_image(
            image_bytes=content, filename=file.filename or ""
        )
    except Exception as e:
        raise HTTPException(
    ...
    content_safety_report = {
        ...
        "safety_summary": {
            ...
            "flagged_categories": [
                category
                for category, data in moderation_results.get("categories", {}).items()
            ...
            "highest_risk_category": (
                ...
                if moderation_results.get("categories")
                else "none"
            ),
        },
    }
...
async def get_moderation_categories(token: Dict[str, Any] = Depends(get_current_token)):
    ...
    categories = {
        "violence": {
            ...
            "detection_methods": [
                "Google Vision API",
                "Computer Vision (blood detection)",
                "Edge analysis",
                "Color analysis",
            ],
            ...
            results.append(
                {
                    ...
                    "flagged_categories": [
                        category
                        for category, data in moderation_results.get(
                            "categories", {}
                        ).items()
                        if data.get("detected", False)
                    ],
    ...
    return {
        ...
        "summary": {
            ...
            "highest_risk_score": max(
                [r.get("overall_score", 0) for r in successful_analyses], default=0
            ),
