...
class ImageAnalysisService:
    ...
    async def _analyze_image_properties(self, image: Image.Image) -> Dict[str, Any]:
        ...
        def _properties_analysis():
            try:
                ...
                if colors:
                    ...
                    red_dominance = (
                        dominant_color[0] / 255.0 if len(dominant_color) >= 3 else 0
                    )
                else:
                    red_dominance = 0
                ...
                violence_score = min(
                    red_dominance * 0.3 + (1 - brightness / 255) * 0.2, 0.5
                )
        ...
        return await asyncio.get_event_loop().run_in_executor(
            self.executor, _properties_analysis
        )
    ...
    def _combine_analysis_results(
        ...
        for category, scores in combined_categories.items():
            if scores:
                ...
                final_categories[category] = {"detected": False, "confidence": 0.05}
