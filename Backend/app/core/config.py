...
class Settings:
    ...
    USE_GPU_ACCELERATION: bool = (
        os.getenv("USE_GPU_ACCELERATION", "false").lower() == "true"
    )
    ...
    # Analysis Thresholds
    CONTENT_SAFETY_THRESHOLD: float = float(
        os.getenv("CONTENT_SAFETY_THRESHOLD", "0.5")
    )
    ...
    # Logging
    LOG_ANALYSIS_RESULTS: bool = (
        os.getenv("LOG_ANALYSIS_RESULTS", "true").lower() == "true"
    )
    LOG_PROCESSING_TIME: bool = (
        os.getenv("LOG_PROCESSING_TIME", "true").lower() == "true"
    )
